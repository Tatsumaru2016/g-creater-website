...
export function ContentAdmin({ open, onClose }: ContentAdminProps) {
  ...
  const grouped = useMemo(() => {
    ...
    return groupFlatKeys(Object.fromEntries(keys.map((k) => [k, ""])));
  }, [copySchema, query, defaults, siteCopy, editLocale]);
    ...
    >
      <div className="w-full max-w-2xl h-full bg-[#0a0a0f] border-l border-cyan-500/20 shadow-[-20px_0_60px_rgba(0,245,255,0.08)] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-6">
          {(Object.entries(grouped) as [string, string[]][]).map(([group, keys]) => (
            <section key={group}>
                    ...
                    >
                      <div className="flex items-start justify-between gap-2 mb-1.5">
                        ...
                        {isOverride && (
                          <button
                            type="button"
                            onClick={() => resetSiteCopyField(editLocale, key)}
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            setSiteCopyField(
                              editLocale,
                              key,
                              editorValueToFlat(e.target.value, effective)
                            )
                          }
        ...
        <footer className="shrink-0 flex flex-wrap items-center gap-2 px-4 py-3 border-t border-white/10 bg-black/40">
          ...
          <button
            ...
            onClick={() => {
              if (
                window.confirm(t("admin.resetLocaleConfirm"))
              ) {
